...
const Home = () => {
  ...
  async function aadharSubmit(values) {
    if (values.aadhar.length != 12) {
      // error invalid aadhar
    }
    await makeGetRequest(values).then(resp => {
      console.log(resp);
    });
    Actions.about();
  }

  async function makeGetRequest(values) {
    // var txid = '';

    // let payload = {
    //   'uidnumber': values.aadhar,
    //   'captchaTxn': state.txnid,
    //   'captchaValue': values.captcha,
    //   'transactionID': txid,
    // };
    console.log('trying')

    // await fetch('http://localhost:5000/otp', {
    //   method: 'POST',
    //   headers: {
    ...
    //     console.length(res)
    //   })
    let res = await axios.get('https://c4df-115-96-219-151.ngrok.io/');
    console.log(res)

    // this.state.result = res.data.status;
    // return res;
  }
          ...
          <View
            ...
            <View style={{flex: 3}}>
              <Image
                ...
                source={{uri: 'data:image/png;base64,' + base64enc}}
              />
            </View>
            <View
              style={{
...
//If result=='Success', then go to next page. else show error alert.
